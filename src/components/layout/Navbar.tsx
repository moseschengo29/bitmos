        ...
        <div className="mx-auto flex h-16 max-w-[1400px] items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <nav className="hidden h-full lg:flex items-center gap-2">
            <div className="flex h-full items-center">
              <Link href="/what-we-do" className="rounded-md px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 hover:text-slate-900">What We Do</Link>
            </div>
            
            {/* 1. SOLUTIONS MEGA MENU */}
            <div className="group flex h-full items-center">
  <button className="flex items-center gap-1 rounded-md px-3 py-2 text-sm font-medium text-slate-600 transition-colors hover:bg-slate-50 hover:text-slate-900">
    Solutions
    <ChevronDown className="h-3 w-3 text-slate-400 transition-transform group-hover:rotate-180" />
  </button>

  <div className="absolute left-0 top-16 w-full border-b border-slate-200 bg-white shadow-xl opacity-0 invisible translate-y-1 transition-all duration-200 ease-out group-hover:opacity-100 group-hover:visible group-hover:translate-y-0">
    <div className="mx-auto w-full max-w-[1400px] px-4 sm:px-6 lg:px-8">
      {/* UPDATED GRID PROPORTIONS: Services column gets 2.2fr to expand its width */}
        ...
        <div className="flex flex-col border-r border-slate-100 pr-8">
          ...
          <div className="flex flex-col gap-3">
            {["Enterprise ERP", "Learner Management", "Financial Tech"].map((item, i) => (
              <Link key={i} href={`/solutions/${generateSlug(item)}`} className="flex items-center gap-3 text-sm font-medium text-slate-700 hover:text-blue-600 transition-colors">
          ...
          <div className="flex flex-col gap-3">
            {[
              ...
              <Link 
                ...
                // ADDED whitespace-nowrap to absolutely force single-line text
        ...
        <div className="flex flex-col border-r border-slate-100 pr-8">
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-4">
            {["Data Analytics", "Pen Testing", "AI Integrations", "System Audits"].map((item, i) => (
              <Link key={i} href={`/solutions/${generateSlug(item)}`} className="flex items-center gap-2 text-sm font-medium text-slate-700 hover:text-blue-600 transition-colors whitespace-nowrap">
        ...
        <div className="flex flex-col pl-2">
          <div className="group/card relative flex h-full w-full flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-md">
            ...
            <div className="flex grow flex-col justify-between p-5">
              ...
              <Link href="/demo" className="mt-4 flex w-fit items-center text-sm font-semibold text-blue-600 hover:text-blue-700">Watch Demo <span className="ml-1 transition-transform group-hover/card:translate-x-1">&rarr;</span></Link>
            </div>
          </div>
        </div>
            ...
            {/* 2. SECTORS MEGA MENU */}
            <div className="flex h-full items-center">
              <Link href="/sectors" className="rounded-md px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 hover:text-slate-900">Sectors</Link>
            </div>
                  ...
                  <div className="grid grid-cols-[1fr_1fr_1fr_1.5fr] gap-8 py-8">
                    <div className="flex flex-col border-r border-slate-100 pr-8">
                      ...
                      {["Technical Blog", "Executive Guides", "Events"].map((item, i) => (
                        <Link key={i} href={`/resources/${generateSlug(item)}`} className="mb-3 text-sm font-medium text-slate-700 hover:text-blue-600">{item}</Link>
                      ))}
                    </div>
                    <div className="flex flex-col border-r border-slate-100 pr-8">
                    ...
                    <div className="flex flex-col border-r border-slate-100 pr-8">
                      ...
                      <Link href="/about-us" className="mb-3 text-sm font-medium text-slate-700 hover:text-blue-600">About Us</Link>
                      <Link href="/careers" className="mb-3 text-sm font-medium text-slate-700 hover:text-blue-600">Careers</Link>
                      <Link href="/contact-us" className="text-sm font-medium text-slate-700 hover:text-blue-600">Contact Us</Link>
                    </div>
                    <div className="flex flex-col pl-2">
                      <div className="group/report relative flex h-full w-full flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-md">
                        ...
                        <div className="flex grow flex-col justify-between p-4 bg-slate-50/50">
                          ...
                          <Link href="/resources/reports/2026-enterprise-cloud-state" className="mt-3 flex w-fit items-center rounded-md bg-white border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 transition-all hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700">
            ...
            <div className="flex h-full items-center">
              <Link href="/case-studies" className="rounded-md px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 hover:text-slate-900">Case Studies</Link>
            </div>
          </nav>
          ...
          <div className="flex items-center gap-4 relative z-[60]">
            <div className="hidden lg:flex items-center gap-3">
              <Link href="/contact-us" className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-blue-700">Start a Project</Link>
            </div>

            {/* Mobile Hamburger Toggle (Made relative and z-60 so it sits above the mobile menu) */}
            <button 
      ...
      >
        ...
        {/* Padding top ensures content starts below the physical navbar */}
